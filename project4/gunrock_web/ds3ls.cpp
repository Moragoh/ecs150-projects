...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  int blockNum = blockOffset + super.inode_region_addr;

  disk->readBlock(blockNum, blockBuffer);

  int withinBlockInodeNum = inum % blockInodeCount;
  inode_t inode = blockBuffer[withinBlockInodeNum]; // Inode for that inum retrived. Now we can get metadata from this
